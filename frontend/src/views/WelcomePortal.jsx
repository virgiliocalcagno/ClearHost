...
export default function WelcomePortal() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (!formData.id_foto) return alert('Por favor sube una foto de tu identificación');
    
    try {
      setSigningUp(true);
      await api.post(`/welcome/${reservaId}/register`, formData);
      await loadInfo(); // Reload to get WiFi/Codes
      setStep('complete');
    } catch (err) {
            ...
            <div className="grid gap-4">
              ...
              <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <span className="font-black text-slate-900 tracking-tighter cursor-copy" onClick={() => {
              ...
              {/* Rules Button */}
              <div className="p-6 border border-slate-200 rounded-[2rem] bg-white flex items-center justify-between group hover:border-slate-400 transition-colors">
                 <div className="flex items-center gap-4">
                    <FileText className="text-slate-400 group-hover:text-slate-900 transition-colors" />
                    <span className="text-sm font-black uppercase tracking-widest">Reglas de la Casa</span>
                 </div>
                 <button className="text-slate-300 group-hover:text-slate-900">→</button>
              </div>
            </div>
          </div>
        )}
